...
public class Day7Part2 {
    ...
    private static boolean solvable(long target, String[] values) {
        double possibilities = Math.pow(3, values.length-1);
        String format = "%" + (values.length-1) + "s";
        BigInteger counter = new BigInteger("0");
        while (counter.intValue() < possibilities) {
            long partial = Long.parseLong(values[0]);
            String strCount = String.format(format, counter.toString(3)).replace(' ', '0');

            for (int i = 0; i < values.length - 1; i++) {
                if (strCount.charAt(i) == '2') {
                    partial = Long.parseLong(partial + values[i+1]);
                }
                else if (strCount.charAt(i) == '1') {
